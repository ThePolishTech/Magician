...
/// is the name of a character's attribute, and the value is an `Option` containing it's value
/// (`String`) when it it known. The second element of the tuple is a `Message` struct that
...
impl TypeMapKey for CharacterBuildingDataKey {
    ...
    type Value = HashMap<u64, (HashMap<String, Option<String>>, Message)>; 
}
